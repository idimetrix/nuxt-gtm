...
module.exports = async function gtmModule (_options) {
  ...
  // Compile scripts
  const injectScript = `var f=d.getElementsByTagName(s)[0],j=d.createElement(s);${options.crossOrigin ? 'j.crossOrigin=\'' + options.crossOrigin + '\';' : ''}${options.scriptDefer || options.scriptAsync ? `j.${options.scriptDefer ? 'defer' : 'async'}=true;` : ''}j.src='${options.scriptURL + '?id=\'+i' + (queryString ? (`+'&${queryString}` + '\'') : '')};f.parentNode.insertBefore(j,f)` // deps: d,s,i
  ...
  if (typeof this.options.head === 'function') {
    // eslint-disable-next-line no-console
    console.warn('[nuxt-gtm] head is provided as a function which is not supported by this module at the moment. Removing user-provided head.')
    this.options.head = {}
  }
